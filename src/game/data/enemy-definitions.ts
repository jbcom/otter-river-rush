export const ENEMY_DEFINITIONS = [
  {
    id: 'lazy_log',
    name: 'Lazy Log',
    description: 'A slow-moving fallen log that drifts predictably downstream. Perfect for beginners to learn timing.',
...

  {
    ...
    description: 'Aggressive turtle that actively chases the player when nearby. Moves slowly but persistently.',
...

  {
    id: 'erratic_fish',
    name: 'Erratic Fish',
    description: 'Unpredictable fish that changes direction randomly. Sometimes flees, sometimes approaches.',
...

  {
    id: 'pike_predator',
    name: 'Pike Predator',
    description: 'Cunning pike that pursues the player intelligently, predicting movement patterns.',
...

  {
    ...
    description: 'Mystical entity that creates whirlpools and uses complex evasion patterns. Alternates between offense and defense.',
...

  {
    id: 'river_kraken',
    name: 'River Kraken',
    description: 'Boss-level tentacled monster with multiple AI phases. Uses flocking behaviors with multiple tentacles and advanced tactical planning.',
    ...
    // Special properties
    specialAbilities: ['tentacle_swarm', 'tidal_wave', 'adaptive_ai', 'multi_phase'],
...
export interface EnemyDefinition {
  ...
  // Optional special ability parameters
  [key: string]: any;
}
...
export function getAvailableEnemies(difficulty: number): EnemyDefinition[] {
  return ENEMY_DEFINITIONS.filter(enemy => 
...
export function selectRandomEnemy(availableEnemies: EnemyDefinition[]): EnemyDefinition {
  const totalWeight = availableEnemies.reduce((sum, enemy) => sum + (enemy.spawnWeight * enemy.spawnProbability), 0);
