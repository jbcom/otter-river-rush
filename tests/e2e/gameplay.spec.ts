...
test.describe('Otter River Rush', () => {
  ...
  test('should show game over screen on collision', async ({ page }) => {
    ...
    await page.waitForTimeout(5000, { timeout: 10000 });
  ...
  test('should show leaderboard', async ({ page }) => {
    const leaderboardButton = page.locator('button', { hasText: /leaderboard/i });
    
    if (await leaderboardButton.isVisible()) {
      await leaderboardButton.click();
      await expect(page.locator('text=/leaderboard|high scores/i')).toBeVisible();
    }
  });

  test('should show settings menu', async ({ page }) => {
    const settingsButton = page.locator('button[aria-label*="settings"], button', { hasText: /settings/i });
  ...
  test('should show achievements', async ({ page }) => {
    const achievementsButton = page.locator('button', { hasText: /achievements/i });
  ...
  test.describe('Performance', () => {
    ...
    test('should have small bundle size', async ({ page }) => {
      ...
      const totalSize = jsFiles.reduce((sum: number, r: any) => sum + r.transferSize, 0);
